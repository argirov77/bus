...
| POST | `/pay` | Два режима: с админским Bearer-токеном помечает бронь как `paid` (офлайн-оплата сотрудником, ответ `204`); без админ-токена возвращает LiqPay payload для внешнего фронтенда (ответ `200`). |
