...
| POST | `/public/purchase` | Публичная «мгновенная покупка»: backend делает `book` + сразу возвращает LiqPay payload для оплаты. |
...
| POST | `/purchase` | Создание бронирования сразу в статусе `paid` (офлайн), только для admin. |
...
- **Внешний фронт (token/public)** может использовать либо `POST /public/purchase` (мгновенный flow: book+pay/LiqPay), либо двухшаговый flow `POST /book` -> `POST /pay`, либо cookie-flow `POST /public/purchase/{purchase_id}/pay`.
...
### Открытая обратка (open-date return)

«Круговая» покупка может включать предоплаченный обратный билет без даты: цена берётся
из того же прайслиста в обратном направлении минус 10% (для льготника дополнительно ×0.95).
Такой ваучер хранится в таблице `open_ticket` (status `open`, срок 3 месяца). Позже в
миникабинете (cookie-сессия из QR) пользователь выбирает рейс и место — создаётся обычный,
уже оплаченный билет.

- Выпуск ваучера: добавьте `"open_return": true` в payload прямого рейса
  `POST /purchase/` или `POST /book` (см. общий payload ниже). `amount_due` заказа
  увеличивается на стоимость открытой обратки для каждого пассажира.
- Круговая скидка 10% для обычной (с датой) обратки: передавайте `"is_return_leg": true`
...
- `is_return_leg=true` → тариф ноги ×0.90; `open_return=true` → плюс цена обратного направления ×0.90.
...
> Для внешнего публичного сценария доступны оба безопасных варианта с LiqPay: `POST /public/purchase` (мгновенный book+pay) или `POST /book` с последующим `POST /pay`.

#### Общий payload для `POST /purchase/`, `POST /book`, `POST /public/purchase`
- **Body:**
```json
{
  "tour_id": 101,
...
```
- `is_return_leg` (опц.): пометить этот рейс как обратный для круговой скидки 10%
  (применяется к тарифу пассажира, ×0.90; стыкуется с льготой ×0.95).
- `open_return` (опц.): выпустить для каждого пассажира предоплаченный обратный билет
  с открытой датой (обратное направление, −10%). Сумма добавляется в `amount_due`.
...
```
- **200 для `POST /public/purchase` (мгновенный book+pay):**
```json
{
  "purchase_id": 555,
  "amount_due": 3200.0,
