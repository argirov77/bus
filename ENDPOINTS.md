...
| POST | `/pay` | Оплата существующего бронирования. **Primary for external frontend**: для non-admin обязателен билетный токен со scope `pay`, привязанный к этому же `purchase_id`; admin может вызывать с bearer JWT. |
