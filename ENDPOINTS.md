...
из того же прайслиста в обратном направлении минус 8% (для льготника дополнительно ×0.95).
...
- Круговая скидка 8% для обычной (с датой) обратки: передавайте `"is_return_leg": true`
...
- `is_return_leg=true` → тариф ноги ×0.92; `open_return=true` → плюс цена обратного направления ×0.92.
...
```
- `is_return_leg` (опц.): пометить этот рейс как обратный для круговой скидки 8%
  (применяется к тарифу пассажира, ×0.92; стыкуется с льготой ×0.95).
- `open_return` (опц.): выпустить для каждого пассажира предоплаченный обратный билет
  с открытой датой (обратное направление, −8%). Сумма добавляется в `amount_due`.
