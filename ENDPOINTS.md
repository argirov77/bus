...
| POST | `/pay` | Админский/manual-платёж существующего бронирования с переводом в `paid`; в `sales.method` записывается офлайн-источник (`offline`) для согласованности с `/purchase/{purchase_id}/pay`. Токен со scope `pay` необязателен, но при его передаче используется для аутентификации. |
