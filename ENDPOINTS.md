...
из того же прайслиста в обратном направлении минус 15% (для льготника дополнительно ×0.95).
...
```
- `is_return_leg` (опц.): пометить этот рейс как обратный для круговой скидки 15%
  (применяется к тарифу пассажира, ×0.85; стыкуется с льготой ×0.95).
- `open_return` (опц.): выпустить для каждого пассажира предоплаченный обратный билет
  с открытой датой (обратное направление, −15%). Сумма добавляется в `amount_due`.
