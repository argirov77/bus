...
#### Автоматическая отправка билета и чека на email
После бронирования и оплаты сервис автоматически отправляет письма покупателю:

- **Билет** — отправляется всегда при создании брони и при оплате. К письму прикладывается
  PDF-версия билета с QR-кодом, а в теле указана ссылка для управления поездкой. Логика
  находится в `backend/services/email.py` (`render_ticket_email`/`send_ticket_email`) и
  запускается фоновыми задачами из `backend/routers/purchase.py` (`_queue_ticket_emails`).
- **Фискальный чек** — отправляется после успешной онлайн-оплаты (LiqPay), когда включена
  интеграция CheckBox (`CHECKBOX_ENABLED=true`). После фискализации покупки
  (`fiscalize_purchase`) письмо с изображением чека (PNG) и ссылкой на него уходит на
  `customer_email`. Отправку можно отключить переменной `FISCAL_RECEIPT_EMAIL_ENABLED=false`,
  сохранив при этом отправку билетов.

Для работы обеих рассылок необходимо настроить SMTP (см. переменные выше). Если SMTP не
настроен, отправка тихо пропускается и не мешает основному потоку покупки.

**Языки.** Каждое письмо мультиязычное: одно сообщение содержит сразу украинский, русский,
болгарский и английский (порядок задаётся `EMAIL_LANGS` в `backend/services/email.py`),
поэтому язык клиента не нужно определять и выбирать. Шаблоны — единые файлы
`backend/templates/emails/ticket.html` и `receipt.html`. Тема письма также комбинированная
(например, `Квиток / Билет / Ticket №… — Maximov Tours`).
