...
docker compose up
```
После запуска приложение будет доступно на `http://localhost:${BACKEND_PORT}`.
По умолчанию используется порт `8000`, его можно изменить в файле `.env`
(переменная `BACKEND_PORT`).
