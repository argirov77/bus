...
Для тестового входа в административный интерфейс после инициализации
базы данных можно использовать учётную запись по умолчанию
(`admin`/`admin`). Эндпоинт `/auth/login` сверяет введённые данные с
таблицей `users` и возвращает JWT-токен, который следует передавать в
заголовке `Authorization` при обращении к защищённым маршрутам.
Переменные окружения `ADMIN_USERNAME`, `ADMIN_PASSWORD` и `ADMIN_TOKEN`
больше не используются и оставлены в `.env.example` лишь для
...
- **пароль:** `admin`
Пароль хранится в базе как bcrypt-хеш. Для входа через API отправьте те же
данные на эндпоинт `/auth/login`. Приложение сравнит логин и пароль с
таблицей `users` и выдаст JWT-токен, который следует передавать в
заголовке `Authorization` при обращении к административным маршрутам.
