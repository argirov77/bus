...
def test_passenger_multiplier_combinations():
    ...
    assert _passenger_multiplier(False, True) == 0.90
    # Discount and round-trip stack multiplicatively.
    assert _passenger_multiplier(True, True) == 0.95 * 0.90
...
def test_open_return_creates_voucher_and_adds_to_amount(monkeypatch):
    ...
    # forward fare 10 + open return 10 * 0.90 = 19.0
    assert amount_due == 19.0
    assert len(specs) == 1
...
def test_open_return_discount_passenger_price(monkeypatch):
    ...
    # forward льгота 10*0.95 = 9.5 ; open return 10*0.95*0.90 = 8.55
    assert amount_due == round(9.5 + round(10 * 0.95 * 0.90, 2), 2)
...
def test_quote_open_return_uses_outbound_price(monkeypatch):
    ...
    result = quote_purchase(data, request=object())
    # forward 10 + open return 10*0.90 = 19.0
    assert result["amount_due"] == 19.0
